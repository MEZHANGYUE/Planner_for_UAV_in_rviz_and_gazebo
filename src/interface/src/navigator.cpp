...
void model_state_cb(const gazebo_msgs::ModelStates::ConstPtr& msg){
    ...
    real_pose = msg->pose[n-1];   //get the last point of UAV
    ...
    Eigen::Vector3d tran = q_odom * Eigen::Vector3d(0.1, 0, 0);  // Vector3d rotate an angle about orientation , get a direct vector
    ...
    q_cam = q_odom * R;  // compute the camera's orientation from UVA body orientation
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber state_sub = nh.subscribe<nav_msgs::Odometry>("/iris_0/mavros/local_position/odom", 1000, odom_cb);
